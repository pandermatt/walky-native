...
struct AppIconTests {
  ...
  func walkersInFront() throws {
    for icon in AppIcons.alternates {
      let json = try String(
        contentsOf: ios.appending(path: "App/\(icon.name!).icon/icon.json"), encoding: .utf8)
      // Matched loosely around the colon on purpose: these documents are
      // generated by `walky-icons`, but opening one in Icon Composer and
      // saving re-writes it in Apple's own spacing (`"image-name" : "..."`).
      // What this test is about is which layer comes first, not how the file
      // is punctuated, and it should not fail for having been opened.
      let walker = try #require(json.range(of: #""image-name"\s*:\s*"walker"#,
                                           options: .regularExpression))
      let stripes = try #require(json.range(of: #""image-name"\s*:\s*"stripes"#,
                                            options: .regularExpression))
